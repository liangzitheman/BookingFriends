...
//        layout.setOrientation(RelativeLayout.VERTICAL).
    ...
    public void resetpwd_check() {                                //确认按钮的监听事件
        if (isUserNameAndPwdValid()) {
            ...
            int result = DataBaseManager.getInstance(this).findUserByNameAndPwd(userName, userPwd_old);
            if(result==1){                                             //返回1说明用户名和密码均正确,继续后续操作
                if(userPwd_new.equals(userPwdCheck)==false){           //两次密码输入不一样
                    ...
                    User mUser = new User();
                    mUser.setAccountName(userName);
                    mUser.setPassword(userPwd_new);

                    boolean flag = DataBaseManager.getInstance(this).updateUser(mUser);
                    if (flag == false) {
...
//                        mUser.pwdresetFlag=1;
    ...
    public boolean isUserNameAndPwdValid() {
        String userName = mAccount.getText().toString().trim();
        //检查用户是否存在
        int count=DataBaseManager.getInstance(this).findUserByName(userName);
        //用户不存在时返回，给出提示文字
        if(count<=0){
